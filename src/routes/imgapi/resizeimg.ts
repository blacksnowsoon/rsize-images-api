  ...
  async function getTheImage(req: express.Request): Promise<string[]> {let generatedFile: string[] = [];
    const imgInfo = {
    ...
    if (validateTheEntries(imgInfo)) {
      ...
      if (fromThumb.length != 0){
        ...
        generatedFile = await createNewImage(fromFull,requestedImage, Number.parseInt(imgInfo.width), Number.parseInt(imgInfo.height));
      }
  ...
  function validateTheEntries(imgInfo: {name: string, width: string, height: string}): boolean {
    return imgInfo.name.length != 0 && Number.parseInt(imgInfo.width) >= 20 && Number.parseInt(imgInfo.height) >= 20;
  }

   // generate image name to match the size of the image
  function generatImageName(imgInfo: {name: string, width: string, height: string}): string {
  ...
  // create a new image
  async function createNewImage(inputImage: string[], outputImage: string, width: number, height: number): Promise<string[]> {
...
export {image, generatImageName,isFileExsistInFull,isFileExsistInThumb,createNewImage, validateTheEntries, fullDir, thumbDir};
